...
class _MethodEmitter:
    ...
    def _stmt_Case(self, node):
        # CASE evaluates the control expression once into a temp, then tests each
        # WHEN's value(s) top-to-bottom, branching to the first match's body
        # block. WHEN values after the first match are never evaluated
        # (short-circuit, matching BBC BASIC V). The bodies are their own CFG
        # blocks, each rejoining after ENDCASE; OTHERWISE catches an unmatched
        # value, and with no OTHERWISE an unmatched value branches past ENDCASE.
        cond_type = node.condition.actualType
        cond_il = _il_type(cond_type)
        is_string = cond_il == "string"
        slot = self._local_slot("__case_%d" % id(node), cond_type)
        self.lower_expression(node.condition)
        self.emit("stloc V_%d" % slot)

        otherwise_label = None
        for clause in node.whenClauses:
            if not clause.statements:
                continue
            body_label = self._block_label(clause.statements[0].block)
            if type(clause).__name__ == "OtherwiseClause":
                otherwise_label = body_label
                continue
            for match in clause.matches:
                self.emit("ldloc V_%d" % slot)
                self.lower_expression(match)
                # Compare like with like: a WHEN value of a different numeric type
                # (e.g. an integer literal against a real control value) must be
                # converted to the control type before ceq.
                if not is_string:
                    self._coerce(cond_il, match.actualType)
                self.emit(_STRING_EQUALS if is_string else "ceq")
                self.emit("brtrue " + body_label)

        # No WHEN matched: run OTHERWISE, or continue after ENDCASE.
        if otherwise_label is not None:
            self.emit("br " + otherwise_label)
        else:
            following = findFollowingStatement(node)
            if following is not None:
                self.emit("br " + self._block_label(following.block))
            else:
                self.emit("ret")  # CASE is the last statement (wrapper rewrites)
