...
""" % (filename, repr(_lr_method), repr(Signature.digest())))
            ...
            import pickle
            with open('lr_action_items.pickle', 'w') as action_items_f:
                pickle.dump(items, action_items_f)
            
            f.write("\n")
            f.write("import pickle\n")
            f.write("_lr_action_items = pickle.load(open('lr_action_items.pickle', 'r'))\n")
...
""")
            ...
            with open('lr_goto_items.pickle', 'w') as goto_items_f:
                pickle.dump(items, goto_items_f)
            
            f.write("\n")
            f.write("_lr_goto_items = pickle.load(open('lr_goto_items.pickle', 'r'))\n")
...
""")
            ...
            productions_for_pickle = []
            for p in Productions:
                if p:
                    if (p.func):
                        productions_for_pickle.append( (p.name, p.len, p.func.__name__,p.file,p.line) )
                    else:
                        productions_for_pickle.append( (p.name, p.len, None, None, None) ) 
                else:
                    products_for_pickle.append( None )
    
            with open('productions.pickle', 'w') as productions_f:
                pickle.dump(productions_for_pickle, productions_f)
            
            f.write("\n")
            f.write("_lr_productions = pickle.load(open('productions.pickle', 'r'))\n")        
    except IOError,e:
